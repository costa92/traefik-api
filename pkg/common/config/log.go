...
type LogConfig struct {
	...
	Level string `toml:"level" json:"level"  mapstructure:"level" yaml:"level"`
	// Output set output file path, can be filepath or stdout|stderr
	Output string `toml:"output" json:"output" mapstructure:"output" yaml:"output"`
	// Encoding sets the logger's encoding. Valid values are "json" and "console". default: json
	Encoding LogEncoding `toml:"encoding" mapstructure:"encoding" json:"encoding" yaml:"encoding"`
	// enable stacktrace
	DisableStacktrace bool `toml:"disable_stacktrace" mapstructure:"disable_stacktrace" json:"disable_stacktrace" yaml:"disable_stacktrace"`
}
